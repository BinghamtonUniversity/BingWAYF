app.get('/api/idps',function(all_idps) {
    gdg = new GrapheneDataGrid({
        ...
        schema:[
            ...
            {label:'Enabled', name: 'enabled',type:'switch',options:[{label:'Disabled',value:false},{label:'Enabled',value:true}],template:"{{#attributes.enabled}}Enabled{{/attributes.enabled}}{{^attributes.enabled}}Disabled{{/attributes.enabled}}"},
            {label:'Debug', name: 'debug',type:'switch',options:[{label:'Disabled',value:false},{label:'Enabled',value:true}],template:"{{#attributes.debug}}Debug Enabled{{/attributes.debug}}{{^attributes.debug}}Debug Disabled{{/attributes.debug}}"},
            {label:'Order', name: 'order'},
        ], 
        data: _.reverse(all_idps)
    }).on("model:view_users",function(grid_event) {
        window.location = '/admin/idps/'+grid_event.model.attributes.id+'/users';
